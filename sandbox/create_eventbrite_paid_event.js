// mindfulina/sandbox/create_eventbrite_paid_event.js
// Script to create a full event with a PAID ticket, add structured content (text & image), set tax info, and publish.
...
const TICKET_PRICE_USD = "25.00"; // Ticket price in USD, e.g., "25.00" for $25.00
...
async function createAndPublishEvent() {
  ...
  const eventTitle = `Paid Test ($${TICKET_PRICE_USD}) (Logo & Struct. Content Img) - (${new Date().toLocaleTimeString()})`;
  const now = new Date();
  ...
  try {
    ...
    // --- Step 3: Create Paid Ticket Class ---
    console.log("\n--- Step 3: Creating Paid Ticket Class ---");
    const ticketClassPayload = {
      ticket_class: {
        name: "Paid General Admission",
        free: false,
        cost: `USD,${TICKET_PRICE_USD.replace('.', '')}`, // Eventbrite expects cost in cents if no decimal, or with decimal as string "USD,25.00"
    ...
    console.log(`Paid ticket class for $${TICKET_PRICE_USD} created successfully.`);

    // --- Step 3.5: Update Event with Tax Settings ---
    console.log("\n--- Step 3.5: Updating Event with Tax Settings ---");
    const updateEventUrl = `https://www.eventbriteapi.com/v3/events/${eventbriteEventId}/`;
    const taxSettingsPayload = {
      event: {
        // For US events, tax_settings often requires specific country/region details
        // or can sometimes be omitted if your Eventbrite account has default tax handling.
        // Let's try with a more explicit (though still potentially basic) structure.
        // If this fails, Eventbrite's specific tax requirements for your region/account need to be checked.
        // An empty object {} might imply default/no tax collection by the organizer.
        tax_settings: {
          // The exact structure can be very specific.
          // For basic "no tax applied by organizer" or "pass-through," an empty object
          // or a specific "type" might be needed. This is often poorly documented.
          // We're trying with {} as it was the initial error message's implication.
        }
      }
    };
    await apiCall(updateEventUrl, 'POST', EVENTBRITE_PRIVATE_TOKEN, taxSettingsPayload);
    console.log("Event updated with tax settings successfully.");

    // --- Step 4: Publish Event ---
    console.log("\n--- Step 4: Publishing Event ---");
    const publishUrl = `https://www.eventbriteapi.com/v3/events/${eventbriteEventId}/publish/`;
    const publishResult = await apiCall(publishUrl, 'POST', EVENTBRITE_PRIVATE_TOKEN); // No body needed for publish
